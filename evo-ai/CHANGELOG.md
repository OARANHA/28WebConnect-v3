...
### Added
- Channels: Always-visible actions menu with Delete option
- Channels: Delete confirmation dialog with extra warning when connected
- Channels: Debug endpoint GET /api/v1/channels/_debug/raw (JWT-protected)

### Changed
- Channels: Normalize Evolution-API status mapping (state/status/connectionStatus, connected/isConnected/online, nested connection/device)
- Channels: Map avatar from Evolution-API (profilePicUrl/profilePictureUrl/etc.)
- Channels: Robust parsing for fetchInstances ({data} or {response})
- Frontend: Optimistic deletion and automatic resync after remove
- Frontend: Fix Channels page build error and UI polish
