...
class Channel(Base):
    """
    Channel model for managing communication channels (WhatsApp, Instagram, Email, SMS)
    Each channel is owned by a client and represents an Evolution-API instance
    """
    ...
    client = relationship("Client", backref="channels")

    __table_args__ = (
        CheckConstraint(
            "channel_type IN ('whatsapp', 'instagram', 'email', 'sms')",
            name="check_channel_type"
        ),
        CheckConstraint(
